...
describe('canvasPerformance', () => {
  ...
  it('커서 파이프라인과 slow frame을 1초 표본으로 집계한다', async () => {
    ...
    performance.recordCanvasPerformanceProjection('full', 250)
    performance.recordCanvasPerformanceProjection('incremental', 2)
    ...
    expect(snapshot.projectionPipeline).toEqual({ fullCollectionScans: 1, incrementalPatches: 1, projectedItems: 252 })
